...
std::string RPNCalc::parseRString(std::istream &input) {
    ...
    while (iter < temp.length() && bracketCount != 0) {
        //if a value is not a valid command
        //return an empty string
        if (!isValidCommand(std::string(temp[iter])))
            return "";
...
bool RPNCalc::isCommand(std::string str) {
    if (str == NOT || str == PRINT || str == CLEAR || str == DROP || str == DUP || str == SWAP)
        return true;
    else return false;
}

bool RPNCalc::isValidCommand(std::string command) {
    if (!isNumber(command) && 
        !isBoolean(command) &&
        !isOperation(command) && 
        !isCommand(command))
