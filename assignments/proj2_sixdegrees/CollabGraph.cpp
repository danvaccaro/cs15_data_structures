...
{
    // check that artist is a valid vertex
    enforce_valid_vertex(artist);
    // declare a vector to store the neighbors
    vector<Artist> result;
    // store the neighbors vector for the input artist
    vector<Edge> vertex_edge_set = graph.at(artist.get_name())->neighbors;
    // loop through the edges and push the neighbor member to the result list
    for (uint i = 0; i < vertex_edge_set.size(); ++i)
    {
        result.push_back(vertex_edge_set[i].neighbor);
    }
    // return the list of neighboring artists
    return result;
}
...
{
    // ensure that both source and dest are valid artists in the graph
    enforce_valid_vertex(source);
    enforce_valid_vertex(dest);
    // initialize the graph for traversal
    for (auto itr = graph.begin(); itr != graph.end(); itr++)
    {
        itr->second->visited = false;
        itr->second->predecessor = nullptr;
    }
    // store the start vertex
    Vertex *start = graph.at(source.get_name());
    // instantiate a queue to hold the vertices
    queue<Vertex *> q;
    ...
    while (!q.empty())
    {
        ...
        // for each neighbor
        for (uint i = 0; i < currNode->neighbors.size(); ++i)
        {
            // define neighbor as a neighboring Vertex in the graph
            Vertex *neighbor = graph.at(currNode->neighbors[i].neighbor.get_name());
            ...
            if (!neighbor->visited)
            {
                ...
                // push the neighbor to the back of the queue
                q.push(neighbor);
            }
        }
    }
    ...
    while (currArtist.get_name() != "")
    {
        ...
        currArtist = get_predecessor(currArtist);
    }
    //return the path
    return path;
}
